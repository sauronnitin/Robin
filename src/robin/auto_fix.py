        ...
        for pattern in ("*.db-journal", "*.sqlite-journal", "*.db-wal"):
            for path in root.rglob(pattern):
                ...
                if not _is_allowed_path(rel) and not rel.startswith("dashboard/"):
                    continue
                # Only allow journal cleanup under src/robin or dashboard (non-secret).
                if not (rel.startswith("src/robin/") or rel.startswith("dashboard/")):
                    continue
                try:
                    path.unlink()
                    cleared.append(rel)
...
def _heal_linkedin_tool_schema() -> tuple[bool, str]:
    ...
    path = _PROJECT_ROOT / "src" / "robin" / "tools" / "linkedin_scout.py"
    if not path.exists():
...
def _llm_plan(issue: dict[str, Any]) -> dict[str, Any] | None:
    ...
    system = (
        ...
        '{"summary":"...","edits":[{"path":"src/robin/...","old":"...","new":"..."}],'
