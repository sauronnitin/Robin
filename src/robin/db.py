...
DB_PATH = _PROJECT_ROOT / "dashboard" / "robin.db"
# Pre-rename filename. A clone created before the JobHunter -> Robin rename has
# its history in the old file; move it across once rather than silently
# starting an empty DB beside it.
_LEGACY_DB_PATH = _PROJECT_ROOT / "dashboard" / "jobhunter.db"
...
def _adopt_legacy_db() -> None:
    ...
    try:
        ...
        print(f"[robin] migrated {_LEGACY_DB_PATH.name} -> {DB_PATH.name}")
    except OSError as exc:
        print(f"[robin] could not migrate legacy DB ({exc}); starting fresh at {DB_PATH.name}")
