...
from robin.profile import profile_resume_path
...
def _hint_files(code: str) -> list[str]:
    if code == "live_rate_limit":
        return ["src/robin/crew.py", "src/robin/config/tasks.yaml"]
    if code == "live_tool_failure":
        return ["src/robin/crew.py", "src/robin/tools/"]
    ...
    return ["dashboard/events.jsonl", "src/robin/crew.py"]
