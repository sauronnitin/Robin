...
from jobhunter_ai.latex_sanitize import is_plausible_latex, sanitize_latex_source
...
class LatexToPdfCompilerInput(BaseModel):
    ...
    latex_source: str = Field(
        ...,
        description="The full LaTeX source code to be compiled into a PDF document.",
    )
...
class LatexToPdfCompiler(BaseTool):
    ...
    description: str = (
        "Compiles a full LaTeX source string into a PDF using the YtoTech LaTeX-on-HTTP API. "
    ...
    def _run(self, latex_source: str) -> str:
        return self._compile(latex_source, allow_base_retry=True)
