...
import type { TextItems, Line, Lines, TextItem } from "lib/parse-resume-from-pdf/types";
  ...
  for (let line of lines) {
    ...
    for (let i = line.length - 1; i > 0; i--) {
      ...
      if (distance <= typicalCharWidth) {
        if (shouldAddSpaceBetweenText(leftItem, currentItem)) {
          leftItem.text += " ";
        }
...
/**
 * Sometimes a space is lost while merging adjacent text items.
 * Bold/italic font switches are the common case: the space glyph is omitted and
 * "Increased" + "engagement" becomes "Increasedengagement".
 */
const shouldAddSpaceBetweenText = (leftItem: TextItem, rightItem: TextItem) => {
  ...
  if (!leftTextEnd || !rightTextStart) return false;
  if (/\s/.test(leftTextEnd) || /\s/.test(rightTextStart)) return false;

  const leftWord = isWordChar(leftTextEnd);
  const rightWord = isWordChar(rightTextStart);

  // Bold / italic / weight change between word characters
  if (
    leftWord &&
    rightWord &&
    String(leftItem.fontName || "") !== String(rightItem.fontName || "")
  ) {
    return true;
  }

  // Same-font jammed short connectors: timeand, handoffsby, andsession
  if (
    leftWord &&
    rightWord &&
    /^(and|by|to|of|in|or|for|with|from|the|a|an|on|at|as|vs)\b/i.test(rightText)
  ) {
    return true;
  }

  // lower → Upper across a merge (common when emphasis ends mid-phrase)
  if (/[a-z]/.test(leftTextEnd) && /[A-Z]/.test(rightTextStart)) {
    return true;
  }
